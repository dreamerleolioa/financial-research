...
實際專案中的 `GraphState` 已經拆成新聞、技術/市場、分析結果、持倉診斷等多組欄位，但 LangGraph 最終仍使用同一個平鋪 state dict。上面的片段保留核心欄位，重點是讓你理解：`TypedDict` 不只是標註輸入輸出，還是整條分析流程共享狀態的契約。
...
FastAPI 的路由函式可以是 `async def`，但**目前本專案大多數 API 與資料庫操作仍是同步模式**：

- API 路由多數仍用一般 `def`
- DB session 使用同步 `Session`
- 真正有用到非同步思維的地方，主要在外部資料抓取的並行控制與未來可擴充的 async 路由

也就是說，`async / await` 在這個專案目前是重要概念，但不是所有路徑都已全面 async 化。
...
這個章節最容易過時，因為 response schema 隨產品功能成長會持續擴充。現在 `AnalyzeResponse` 已不只是回傳快照與信心分數，還會帶結構化分析、技術指標顯性輸出、基本面資料、策略欄位與錯誤列表。
...
目前不只分析 API，而是由主 app 加上多個 router 組成：

- 分析端點：`POST /analyze`、`POST /analyze/position`
- 內部排程端點：`POST /internal/fetch-raw-data`
- 認證端點：`POST /auth/google`、`POST /auth/google/code`、`GET /auth/me`
- 持倉端點：`GET /portfolio`、`POST /portfolio`、`GET /portfolio/{id}/history`
- 健康檢查：`GET /health`

這個階段比較接近小型產品後端，而不是只有單一 demo API。
...
# Depends：依賴注入，每次請求自動取得 DB session、目前使用者或 graph
...
先前 FastAPI 很常用 `@app.on_event("startup")`，但本專案目前已改成 `lifespan` 模式管理啟動階段工作。這種寫法比較符合新版本 FastAPI 的生命週期模型，也更方便把初始化和清理邏輯集中在一起。

`Depends` 是 FastAPI 的依賴注入機制，適合處理「每次請求都需要的共用資源」（DB 連線、圖物件、目前登入使用者等）。

目前專案裡最常見的依賴有三種：

- `Depends(get_db)`：取得 DB session
- `Depends(get_current_user)`：驗證 JWT 並取得目前登入使用者
- `Depends(get_graph)`：取得分析流程 graph
...
`LangChainAnalyzer` 呼叫 Claude（主）或 GPT（備援）做多維度股票分析，輸出結構化 JSON（`AnalysisDetail`）。目前 Prompt 已經明確拆出技術面、籌碼面、消息面、基本面估值四個獨立段落，並要求最後再給出整合仲裁的 `final_verdict`。
...
本專案的 system prompt 強制 LLM 分四步驟分析，並限制各維度（技術面、籌碼面、消息面、基本面）不得互相混寫，避免 LLM 幻覺跨維度引用不存在的數據。

目前技術面 Prompt 已經明確允許引用下列訊號：
...
另外，LLM 呼叫前還會把昨日 DB 讀出的歷史上下文注入 Prompt，例如：
...
- `prev_ma_alignment`
- `prev_macd_bias`
- `prev_bollinger_position`

這樣做的目的不是讓 LLM 自己「猜昨天發生什麼事」，而是強制它在有明確歷史資料時說明今天訊號是延續還是轉向。
...
整個股票分析流程是一個 LangGraph `StateGraph`，各步驟是節點，資料共享 `GraphState`。和早期版本相比，現在流程已經把「外部資料抓取」拆成獨立節點，並加入 retry loop 與 news refresh 判斷。
...
```mermaid
flowchart TD
    start([START]) --> crawl[crawl]
    crawl --> external[fetch_external_data]
    external --> judge{data_sufficient?}
    judge -->|yes| clean[clean]
    clean --> quality[quality_gate]
    quality --> preprocess[preprocess]
    preprocess --> score[score]
    score --> analyze[analyze]
    analyze --> strategy[strategy]
    strategy --> endNode([END])
    judge -->|no + requires_news_refresh| fetchNews[fetch_news]
    fetchNews --> increment[increment_retry]
    increment --> crawl
    judge -->|no + stale data only| increment
```

`fetch_external_data` 目前會並行抓兩類資料：

- 籌碼面資料（institutional flow）
- 基本面估值資料（fundamental data）

因此現在的 LangGraph 不是只有「股價 + 新聞 + LLM」，而是已經把 rule-based 技術分數、籌碼、基本面與新聞品質檢查都整合進同一條工作流。
...
後端目前約有 31 個測試檔案，涵蓋 API 端點、LangGraph 節點、技術面指標、歷史回補、基本面 provider、持倉診斷、資料庫模型與資料來源。使用 `unittest.mock` 做依賴隔離，避免測試時真的呼叫外部 API。
...
補充：目前 `backend` 採用的是**同步 SQLAlchemy Session**，不是 AsyncSession。這點要和前面 `async / await` 章節一起理解，不要把概念和實際落地狀態混在一起。
...
最近這個專案最值得記住的實戰經驗，是 migration 不只是新增欄位，還常常包含：

- rename 舊欄位
- add 新欄位
- backfill 舊資料
- 先跑 migration，再部署讀新欄位的程式碼
...
補充：現行 API 啟動時已改用 FastAPI `lifespan` 來包 migration，而不是早期的 `@app.on_event("startup")`。觀念不變，但若你對照原始碼，會看到生命週期寫法已經更新。

**近期實戰案例：單一 `is_final` 拆成雙旗標**
...
正確做法不是只改 ORM model，而是要：

1. 對既有表 rename 欄位
2. 對新表 add column
3. 對舊資料做 backfill
4. 確認 n8n / API / script 都改讀新欄位

這類變更正是 Alembic 最有價值的場景。
...
- 前端直接傳 `id_token`
- 前端傳 Google auth code，由後端去交換 token
...
- `POST /auth/google/code`：支援 auth code exchange 的登入入口
- `GET /auth/me`：前端初始化頁面時查目前登入使用者
...
def get_current_user(
    ...
    payload = decode_access_token(token)  # 驗證 JWT
...
使用方式：在任何需要登入的路由加上 `current_user: User = Depends(get_current_user)`，像持倉查詢與持倉歷史 API 都是這樣保護的。
