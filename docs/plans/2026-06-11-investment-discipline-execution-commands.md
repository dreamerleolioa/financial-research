...
2. 新增 cloud internal API：POST /internal/daily-radar/forward-validation/run。
3. 支援 fixture/local mode 與 cloud production mode；正式 validation 來源是 production DB，不是本地 fixture。
4. 支援 `mode: due`，每日盤點成熟的 5、10、20 trading-day windows。
5. validation write path idempotent，不重複寫同一 candidate/window/version。
6. report 至少包含 metadata、sample_summary、bucket_outcomes、rule_outcomes、risk_label_outcomes、market_regime_outcomes、score_decile_outcomes、ablation_candidates、skip_reasons、version_manifest。
7. 不改 live scoring、ranking、bucket 判定、risk penalty 或 scoring/rule version。
8. 新增/更新測試，證明 outcome math、missing data、grouping、report determinism、API due-window selection、idempotency。

完成後停止，回報 changed files、驗證命令、fixture report 摘要、cloud API 觸發方式、production data 限制、下一個建議 phase。
```
...
DB debug mode can read persisted candidates and final raw rows, but local output is still a development artifact:
...
Expected daily cloud trigger after Phase 1:
...
Expected monthly cloud rule-review trigger after Phase 3:
...
Official monthly reports should be generated by cloud backend from production DB and delivered as GitHub Actions artifacts or equivalent downloadable artifacts. Local CLI output is only for development, fixture replay, debug, and regression tests.

Local fixture ablation replay after Phase 3:
...
GitHub Actions artifact delivery after Phase 3:
...
The monthly rule-review report contains automated recommendations only. A later human-approved versioned strategy update plan is required before any live scoring, ranking, rule tier, scoring version, or rule version change.
...
Frontend build/typecheck for Phase 6 UI/copy guard changes:
...
- `.github/workflows/investment-discipline-release-gate.yml`
- Runs backend release-gate pytest set and frontend build.
- Does not call cloud internal APIs; production forward-validation and monthly rule-review artifacts remain separate cloud workflows.
