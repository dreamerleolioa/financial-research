...
對應 spec：`docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §5.1
...
[
  {
    ...
    "expected_checks": [
      "json_valid",
      "no_cross_dimension",
      "verdict_conviction_align",
      "no_overconfident_language",
...
對應 spec：`docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §5.2
...
對應 spec：`docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §6（已否決）
    ...
    <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950 px-4 py-3 flex items-center justify-between gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-text-secondary">
          ...
          {pct != null && (
            <span
              className={`ml-1 font-medium ${parseFloat(pct) >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
              {parseFloat(pct) >= 0 ? "+" : ""}
              {pct}%
            </span>
      ...
      <a href="/portfolio" className="text-xs text-text-muted underline hover:text-text-secondary">
        查看持股診斷
      </a>
    </div>
  );
...
```tsx
{
  (() => {
    ...
    return <HeldPositionBanner item={held} currentPrice={result.snapshot?.current_price} />;
  })();
}
```
...
### 對照 `docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §5.1
...
### 對照 `docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §5.2
...
### 對照 `docs/specs/ai-stock-sentinel-execution-roadmap-spec.md` §6
