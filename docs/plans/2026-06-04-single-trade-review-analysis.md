...
The user-facing review sections are nested under `review_result`. `review_result.user_readable_conclusion` is the frontend source for `交易檢討結論`; it contains `overall_verdict`, `overall_verdict_label`, `one_sentence_reason`, `evidence`, and `next_time_rules`. `overall_verdict` is one of `early`, `reasonable`, `late`, or `insufficient`. `evidence_payload` is persisted and returned separately so the frontend can copy objective evidence JSON. The current implementation sets `llm_summary` to `null` and does not call an LLM.
...
Current implementation note: `review_result.user_readable_conclusion` is deterministic and rule-based. It does not call an LLM, even for `one_sentence_reason`, `evidence`, or `next_time_rules`.
...
- In a future version, generate optional narrative text from already-computed tags and metrics.
...
- `review_result.user_readable_conclusion` is an additive key inside existing JSONB. It needs no migration and does not require a `trade-review-v1` version bump.
...
- Show `review_result.user_readable_conclusion` first as `交易檢討結論`.
