...
Review 行情與正式分析行情有不同寫入邊界。Single Trade Review 的 provider backfill 只能建立 request-scoped snapshot，失敗時可唯讀 fallback 到 `StockRawData`，但不得寫回 canonical row。外部下載必須具備明確 timeout、process-wide non-blocking bulkhead、日期排序去重，以及成功／失敗分離的 refresh TTL；容量耗盡必須釋放主流程並留下 missing reason。Trade v3 與 lifecycle v2 的 evidence 都保存 compact market snapshot、ruleset 與 source fingerprint；fingerprint 必須涵蓋 review-relevant portfolio/event facts、plan provenance、market snapshot 與 lifecycle shared-context replay trace，取代只比較 event/plan `updated_at` 的 freshness 判斷。相同 review key 的 POST 以 portfolio row lock 序列化；未知 lifecycle review version 維持唯讀，不由舊程式建立 v2 或覆寫。
