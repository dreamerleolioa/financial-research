...
| **技術面 (Technical)**     | MA5/20/60 均線、乖離率 (BIAS)、RSI、布林通道、MACD、成交量變化、支撐壓力位                                                           | yfinance + Pandas 計算                                           |
...
- `derive_technical_score(closes, rsi, bias, macd_data=None, bb=None) -> int`：RSI / BIAS / MA 排列 / MACD / 布林通道五項訊號獨立加權，總分映射至 [30, 70]，資料不足時以現有可得訊號計分，全部不足則回傳 50
...
- 目的：將 RSI、BIAS、MA、布林通道、MACD、Institutional Flow 等術語映射為投資者可讀的直白中文
...
> ⚠️ **實作說明（2026-05-18）**：規格原本規劃 `quantify_to_narrative()` 單一函式與 `TERM_ALIAS` 常數，實際落地改為多個獨立 helper：`_bias_narrative()`、`_rsi_narrative()`、`_ma_narrative()`、`_volume_narrative()`、`_bollinger_narrative()`、`_macd_narrative()`、`_inst_narrative()`，統一由 `generate_technical_context()` 協調呼叫。`TERM_ALIAS` 常數未採用，閾值判斷與敘事字串直接內嵌於各 helper。功能等價，可測試性相同。
