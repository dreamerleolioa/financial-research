...
- Lifecycle compact evidence 將有日期的 trailing series 攤平成單日 bars；同日 outer bar 若只有 partial OHLCV，只能逐欄合併非空值，不得刪除已捕捉的完整行情欄位，以確保 evidence 與 source fingerprint 可重建實際輸入。
