...
| `active_etf_holding_snapshots` | 每檔基金每個 MoneyDJ 來源宣告日期的一份 canonical 股票持股快照，保存 source trace、payload hash、parser version 與非股票工具略過數；legacy verification 欄位為滾動部署相容保留，新資料固定為 `single_source` / `1` / `moneydj_only` |
| `active_etf_holdings` | 快照內正規化股票代號、名稱、股數、權重與來源排序；每日增減由同基金相鄰快照 read-time 推導，不重複持久化 diff |
| `active_etf_source_observations` / `active_etf_source_holdings` | MoneyDJ 保存資料日、URL、擷取時間、parser、SHA-256、gzip 原始 payload 與正規化持股，供事後重播；切換前的官方歷史列不破壞性刪除，但 runtime 與 public read 不再使用 |
...
- **基本面（估值）**
  ...
  - Provider 架構：`finmind_only` 保留既有行為；`official_cache_first` 優先讀本地版本庫、缺歷史才 bootstrap；`official_cache_only` 完全不呼叫 FinMind/yfinance
...
- **即時新聞（消息面）**
  ...
  - RSS 預設抓取最多 5 篇，`clean_node` 對每篇各自產生 `CleanedNews`，再聚合成單一 `cleaned_news`
  - 聚合欄位包含：`sentiment_counts`、`sentiment_score`、`sentiment_strength`；`sentiment_strength` 僅調整 rule-based 消息面權重，不改變 LLM 輸出的三態標籤
...
- **技術面指標**（由 `yfinance` 拉取 OHLCV 後，以 **Pandas** 計算）
  ...
  - `KD`：9 日隨機指標，追蹤高低檔交叉與短線反轉時機
  - `ADX`：14 日趨勢強度，ADX < 20 時趨勢訊號需降權，ADX ≥ 25 時趨勢訊號可信度提高
  - `OBV`：能量潮，判斷量價確認或背離
  - `ATR`：14 日平均真實波幅，判斷波動水準並校準停損/停利距離；高波動時降低追價信心
  - `MFI`：14 日資金流量指標，結合價格與成交量判斷資金過熱、低檔或偏多/偏弱
  - `Donchian Channel`：20 日高低價通道，判斷有效突破、跌破或區間上/下半部位置
...
- **籌碼面（法人動向）**
  ...
  - 法人連續買超 / 連續賣超天數
  - 主導買方 / 主導賣方（foreign / trust / dealer / mixed / none）
  - 融資餘額變化（Margin Balance Delta）
  - 融券餘額變化（Short Balance Delta）
  - 借券成交量與變化
  - 外資持股比例與變化
  - 大戶持股比例、大戶/散戶持股比例變化（資料可得時）
  - 三大法人合計買賣超占近期均量比例：`abs(three_party_net) / avg_daily_volume × 100`，用於區分「張數大但相對規模小」與「流動性小型股的重大籌碼事件」
...
**原則**：各維度訊號必須相互印證，訊號衝突時內部訊號強度應主動調降，前台文案應轉成等級、理由與風險提示。

| 情境                                     | 處理邏輯                                         |
| ---------------------------------------- | ------------------------------------------------ |
| 新聞利多 ＋ 外資連買 ＋ RSI 未超買       | 內部訊號強度 ↑，標記「多方共振」                 |
| 新聞利多 ＋ 外資大賣 ＋ 融資增加         | 內部訊號強度 ↓，標記「消息與法人背離，散戶追高風險」 |
| 新聞中性 ＋ 技術均線多頭排列 ＋ 籌碼沉澱 | 內部訊號強度維持，標記「靜默吸籌」               |
| 新聞利空 ＋ 投信連買 ＋ RSI 超賣         | 內部訊號強度微升，標記「逆勢佈局觀察」           |
...
- `news_sentiment` 由 Cleaner 對多篇新聞結構化後聚合輸出（`sentiment_label: positive / negative / neutral` + `sentiment_strength`），**不由 LLM 在分析時自行判斷**
...
| 維度               | 值                                                                             | 分數                                                                |
| ------------------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| `news_sentiment`   | positive / negative / neutral                                                  | +5 / -5 / 0，並乘上 `sentiment_strength`（多篇一致時最高約 1.6 倍） |
...
- `derive_technical_score(closes, rsi, bias, macd_data=None, bb=None, kd_data=None, adx_data=None, obv_data=None, atr_data=None, mfi_data=None, donchian_data=None) -> int`：RSI / BIAS / MA 排列 / MACD / 布林通道 / KD / ADX / OBV / MFI / Donchian 訊號獨立加權，ATR 高波動作為分數折減濾網；總分仍夾在 -5 ~ +5 後映射至約 [33, 67]，避免技術面單維過度放大；資料不足時以現有可得訊號計分，全部不足則回傳 50
- KD 規則：低檔黃金交叉（`bullish_cross` + `oversold`）加分；高檔死亡交叉（`bearish_cross` + `overbought`）扣分
- ADX 規則：`ADX >= 25` 且方向明確時加權趨勢方向；`ADX < 20` 視為盤整，技術分數打折
- OBV 規則：量價同步上升加分；價漲量縮背離或量價同步轉弱扣分；股價下跌但 OBV 未轉弱視為承接力道
- MFI 規則：`bullish_flow` / `oversold` 加分；`bearish_flow` / `overbought` 扣分
- Donchian 規則：`breakout_up` 加分；`breakdown_down` 扣分
- ATR 規則：`volatility_level = high` 時將技術分數打折，避免高波動環境過度追價
...
- 目的：將 RSI、BIAS、MA、布林通道、MACD、KD、ADX、OBV、ATR、MFI、Donchian Channel、Institutional Flow 等術語映射為投資者可讀的直白中文
...
> ⚠️ **實作說明（2026-05-25）**：規格原本規劃 `quantify_to_narrative()` 單一函式與 `TERM_ALIAS` 常數，實際落地改為多個獨立 helper：`_bias_narrative()`、`_rsi_narrative()`、`_ma_narrative()`、`_volume_narrative()`、`_bollinger_narrative()`、`_macd_narrative()`、`_kd_narrative()`、`_adx_narrative()`、`_obv_narrative()`、`_atr_narrative()`、`_mfi_narrative()`、`_donchian_narrative()`、`_inst_narrative()`，統一由 `generate_technical_context()` 協調呼叫。`TERM_ALIAS` 常數未採用，閾值判斷與敘事字串直接內嵌於各 helper。功能等價，可測試性相同。
...
1. `FinMindProvider`（Primary）：欄位完整（含三大法人、連續買賣超、主導買賣方、融資融券、借券、外資持股、大戶/散戶持股結構；部分資料集權限不足時以 warning 降級）
...
- 敘事層額外使用 `avg_daily_volume` 計算法人買賣超相對規模；此欄位不改變 provider 原始 schema，只由 `preprocess_node` 以 yfinance 近期成交量補入 context。
...
> **LLM input 穩定化（2026-05-25）**：`analyze_node` 在呼叫 LLM 前必須組出 compact `signal_summary`，以 JSON 字串形式放在 prompt 最前段，作為 rule-based labels 的唯一優先入口。此摘要包含 `technical_signal`、`institutional_flow`、`sentiment_label`、`confidence_score`、`cross_validation_note`、KD / ADX / OBV / ATR / MFI / Donchian、MACD、布林位階、RSI、支撐壓力、籌碼主導者、融資融券、借券、持股結構、消息聚合強度與策略標籤。LLM 可解釋這些欄位，但不得改寫 labels 或重新計算分數。
...
4. 讀取 `preprocess_node` 轉換後的技術面敘述與 `signal_summary.technical_evidence`，理解均線、RSI、布林、MACD、KD、ADX、OBV、ATR、MFI、Donchian 與支撐壓力語義
...
> **設計動機**：目前 LLM 將三維資訊（新聞、技術、籌碼）揉雜在單一 `summary` 段落中，造成「資訊密度過高」且「推論過程像黑盒子」。分維度拆解不僅提升可讀性，更強化系統的可解釋性，使前端能顯示每個維度的「維度燈號」並讓使用者一眼看出哪個維度在拖累或支撐策略判斷。
...
```json
{
  ...
  "cross_validation_note": "外資連買 3 日，與新聞利多訊號一致，訊號強度偏高",
  "risks": ["RSI 接近超買區間 (>70)，短線需注意回測"],
  "data_sources": ["google-news-rss", "yfinance", "twse-openapi"],
  "tech_insight": "均線維持多頭排列，MACD 位於零軸上方且偏多，股價貼近布林上軌，短線續強但追價風險升高。",
...
> - `final_verdict`：綜合仲裁段落，解釋三維訊號如何導向當前訊號強度與策略；此段允許跨維度整合推論，但預設不應引用 raw score 作為 headline
  ...
  "properties": {
    ...
    "action_plan": {
      ...
      "properties": {
        "action": {
          "type": "string",
          "description": "帶部位比例的操作建議，如「分批佈局（首筆 30%）」"
        },
        "target_zone": { "type": "string" },
        "defense_line": { "type": "string" },
        "momentum_expectation": {
          "type": "string",
          "description": "基本動能標籤，有 resistance_20d/support_20d 時附加 If-Then 條件句"
        },
        "breakeven_note": {
          "type": ["string", "null"],
          "description": "mid_term 時輸出獲利 5% 保本提示；short_term / defensive_wait 為 null"
        }
      }
    },
    ...
    "tech_insight": {
      "type": ["string", "null"],
      "description": "技術面獨立分析段落（均線排列、RSI 位階、布林通道、MACD、支撐壓力）；禁止混入籌碼或消息面資訊"
    },
    "inst_insight": {
      "type": ["string", "null"],
...
> **`/analyze` 與 `/analyze/position` 的策略語義邊界（2026-03-16 新增）**
>
> - `/analyze` 頁底部策略區塊的正式定位為「新倉策略建議」，用途是回答「若現在評估開新倉，應如何觀察、等待與分批進場」
> - `/analyze` **不得**被定義為持股中的「當前操作」建議，因該流程缺少成本價、部位大小、持有期間與風險承受度等持倉上下文
> - `/analyze/position` 才是「持股操作建議」唯一真相來源，用於續抱 / 減碼 / 出場等持倉決策
> - `action_plan`、`entry_zone`、`stop_loss`、`holding_period` 皆屬 rule-based 新倉策略輸出；LLM 可參與前段新聞/摘要訊號整理，但**不得直接生成最終進場指令**
...
- Trade Review 與 Lifecycle Review 保留原始 final rows 作 deterministic 計算，但持久化的 `StockRawData` evidence 必須 compact。有日期的 trailing series 與 Daily Radar `technical.price_history` 攤平成單日 bars，且所有 dated series 必須先於 outer bars 合併；同日重疊歷史以較新非空值更新，較新 series 缺少的欄位仍保留既有 completed 值，outer bar 則只能補既有 bar 的缺少欄位，不得用盤中／partial outer quote 覆蓋已捕捉的完整行情。可作 fallback 的 persisted MA20、MA60、RSI14、量比也要進入 compact bar 與 fingerprint。無日期的重複 trailing history 只保存最長一份，避免 evidence payload 隨 outer rows 成倍膨脹，並確保 source fingerprint 涵蓋所有可被分析選用的輸入。
...
Review 行情與正式分析行情有不同寫入邊界。Single Trade Review 的 provider backfill 只能建立 request-scoped snapshot，失敗或 coverage 較差時可唯讀 fallback 到 final `StockRawData`，但不得寫回 canonical row。Position Lifecycle Review 不呼叫 provider，只讀 Daily Radar／正式 final `StockRawData`；先使用 completed `recent_*`，不足時讀取 dated `technical.price_history`，最後才使用資料日早於事件日的 persisted indicators。外部下載必須具備明確 timeout、process-wide non-blocking bulkhead、同 review key 的 process-local non-blocking single-flight、日期排序去重，以及成功／失敗分離的 refresh TTL；重複 refresh 不等待同步 worker，回 `409` 與 `Retry-After`，backend 透過 CORS 暴露該 header，first-party frontend 依 header 有界重試，容量耗盡則釋放主流程並留下 missing reason。一般 snapshot producer 同步保存最後有效 Close 與獨立 Close／High／Low／Volume 日期；legacy row 缺少日期時只保守移除各個無法證明完成的 OHLC 尾端值，不丟棄更早歷史。Provider I/O 前不得持有 DB row lock 或占用 transaction connection；snapshot 完成後才以短 row-lock transaction 重新讀取並寫入，並以品質、normalized usable trading-bar count、Close-owned 完整 covered-date set、持有期間關鍵日期集合、可證明的日期範圍與 fetched-at 共同仲裁，避免過期／非 final／空 row、週末 observation、持有區間缺口、內部日期缺口或較舊 refresh 覆寫較完整結果。正常 provider 的 90% 總 coverage 容忍不得移除既有持有期間日期；只有少於 60 根的 `market-coverage-v1` evidence，或少於 60 根且具有實際 bars、可推導 coverage 的 legacy evidence，才可進行 material fallback 自癒，只有不相容 row count 的 evidence 不得放寬；一旦符合 material recovery，無日期的 estimated fallback 可取代少量 holding dates，避免持有日期保護反向鎖死低品質 evidence。首次建立與後續 refresh 都必須比較 provider／fallback coverage；outer row count 只作 evidence，不得當成跨 provider coverage 單位。Trade v3 與 lifecycle v5 的 evidence 都保存 compact market snapshot、ruleset 與 source fingerprint；目前 ruleset 分別為 `trade-review-ruleset-v3.1` 與 `position-lifecycle-ruleset-v5`，舊 ruleset 不得被 cache reuse。Lifecycle v5 fingerprint 另涵蓋 matched-cash-flow benchmark snapshot；benchmark 每筆現金流只可使用事件日前一根 completed bar。Fingerprint 必須涵蓋 review-relevant portfolio/event facts、plan provenance、實際採用的 market history／indicators 與 lifecycle shared-context replay trace，取代只比較 event/plan `updated_at` 的 freshness 判斷。成功 refresh 即使 content fingerprint 未變，也必須以較新的 `fetched_at` 推進 TTL，且不得清除既有 LLM summary。未知 lifecycle review version 維持唯讀，不由舊程式建立 v5 或覆寫。
...
Portfolio 寫入必須在 application commit 前符合 PostgreSQL 實際欄位 precision 與 scale，不能只依 SQLite 測試、`INTEGER` 股數上限或數值 min/max 推定可保存。Request schema 使用 Decimal 契約限制 `max_digits` / `decimal_places`：`user_portfolio` 的進出場價格範圍為 `0.01`–`99,999,999.99` 且最多 2 位小數；event fees/taxes 上限為 `99,999,999.99` 且最多 2 位小數；lifecycle plan 依 `NUMERIC(12,4)`、`NUMERIC(12,2)`、`NUMERIC(8,4)` 各自限制。加碼與結案的自動費稅、平均成本、損益與報酬率必須在 mutation 前依目標 scale 明確量化，再驗證絕對上限；超界回傳 `422` 且不得留下 portfolio/event 部分寫入。同一輸入若同時寫入 portfolio row 與 event，必須使用同一個已驗證／量化的 Decimal fact，禁止依賴兩種 `NUMERIC` scale 各自隱式四捨五入。
...
- 使用者自報的 yes / partial / no 只保留為 `declared_plan_adherence_score` trace。Lifecycle v5 的獨立客觀觀測器只可核對 event-time fixed-option plan 與 event/market facts；單一可評估 check 只顯示 limited evidence，至少兩項才可輸出 `observed_plan_adherence_score` 與相容 `plan_adherence_score`。`decision_quality_score` 仍為 `null`，不得用損益結果替代決策品質。預設 UI 同時呈現逐項 check、coverage caveat、tier、label 與 source event。
...
- 若使用者需要「續抱 / 減碼 / 出場」判斷，必須導向持股診斷流程，不得在 Analyze 頁以同一卡片語意混用
...
5. **分維度分析卡片（Session 8 新增）**
   ...
   - 視覺目標：使用者一眼看出哪個維度在拖累或支撐策略；避免讓未校準 raw 分數成為主要決策訊號
...
- Raw score visibility boundary：`confidence_score`、`signal_confidence`、`data_confidence` 等 raw values 可供 API、guardrail、回測與 advanced trace 使用；前台預設應以 signal tier、資料品質提示、cross-validation reason、風險與限制條件呈現，不得暗示勝率或投資建議強度。
