...
| 我的持股     | `/portfolio`        | `GET /portfolio` + `GET /portfolio/{id}/history` | active 持股列表 + 個股歷史診斷紀錄 |
| 已結案持股   | `/portfolio/closed` | `GET /portfolio/closed`                          | 已結案紀錄、期間篩選與已實現損益 |
| Daily Radar  | `/daily-radar`      | `GET /daily-radar/latest`                        | 每日觀察候選清單                 |
...
`/portfolio` 只顯示 `GET /portfolio` 回傳的 active 持股，不混入已結案紀錄。
...
頁面載入時呼叫 `GET /portfolio/closed`，只顯示已結案持股。
...
- 依 `exit_date` 套用期間篩選。
- 顯示篩選後的已實現損益總計。
- 顯示已結案持股列表，包含 symbol、進場價、出場價、出場股數、出場日期、已實現損益、報酬率、持有天數與備註。
- 已結案紀錄不出現在 `/portfolio` active 清單。
