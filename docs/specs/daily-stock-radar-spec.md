...
| TWSE/TPEX official market bars | 台灣時間 18:30 以兩個 market-wide request 歸檔當日未還原 OHLCV；手動 backfill 最多 180 個 calendar days。`official_first` 只在本地資料不足或最新日不符時讓 selected symbols 回退 yfinance；ETF/權證等非四位數普通股仍走既有 fallback。 |
...
| local-first selected-symbol OHLCV | 正式排程台灣時間 22:30 執行。`official_first` 先讀 `taiwan_daily_bars`，至少 60 根且最新日等於 `run_date` 才視為完整；不足的 selected symbols 才交給 yfinance batch。`yfinance_only` 保留舊行為，`official_only` 缺資料時明確失敗，不靜默外抓。 |
