...
15. Replay identity 必須要求 validation row 的 `signal_date`、candidate snapshot 的 `record_date` 與 replay record 的 `record_date` 三者完全一致；任一缺漏或不一致都以 `replay_input_incomplete` fail closed，錯日期的 forward outcome 不得進入 candidate config 或 counterfactual governance。
