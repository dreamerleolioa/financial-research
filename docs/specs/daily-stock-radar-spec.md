...
| FinMind Phase 1 Daily AVWAP | 只對 selected universe symbols 做 single-symbol `TaiwanStockPrice` refresh；正式排程台灣時間 19:00 執行。同一 `data_date` 已有 fresh snapshot 時直接重用。若 FinMind 尚未提供 requested `run_date` row，寫入 missing snapshot trace，不阻塞 Daily Radar persistence。 |
...
| Request budget | TWSE RWD fund reports 用 report-level 查詢建立法人 tracks；selected symbols capped 250；FinMind AVWAP / lending / full margin 拆成不同小時，各自只對 selected symbols 做 single-symbol refresh；lending / full-margin 會重用同日 fresh cache；yfinance 只對缺資料的 selected symbols 做一次 batch download，並回寫 prepared universe 技術面 tracks；market index 只抓固定 benchmark；weekly major holders 由週頻背景 endpoint 更新 cache |
