...
| 評分模型 | 需要 bucket 分數、綜合觀察分數、風險扣分與可追溯規則 |
...
| 法人買賣超 | FinMind institutional flow | 外資、投信、自營商方向與連續性 |
| 融資融券 | FinMind margin data | 散戶追價、融資風險、軋空或籌碼壓力觀察 |
...
目前 live run 不從全市場靜態清單開始逐檔掃描，而是由後端自行建立「雙軌 universe」後再進入資料補齊與評分。Universe 來源如下，最後以出現順序保留、去重後形成當日 selected symbols：

1. Same-day institutional leaders：讀取當日外資 top 50 與投信 top 50，先各取 50，再合併去重。
2. Recent accumulation/concentration leaders：讀取最近 5 個交易日的累積與集中度排名 top 50。
3. Final selected universe：依上述軌道順序合併，保留第一次出現的位置，重複 symbol 只留一筆。

這代表 live run 的 universe 上限大約是 150 檔，實際數量會因外資、投信與近期累積名單重疊而更低。MVP 原始設計仍排除 ETF、權證、特別股與資料欄位明顯不完整標的，但目前 live default 是雙軌選出的候選 universe，不是完整上市櫃全市場逐檔掃描。
...
目前已落地的 live run 是自包含流程：`POST /internal/daily-radar/run` 先呼叫雙軌 universe 選股，再對 selected symbols 補齊缺少的 OHLCV，接著執行 Stage 1/2 rule-based scoring，最後寫入 run log 與 candidates。公開 Daily Radar 讀取端點與 response schema 不因這個後端流程改變。

### 4.3 外部資料 request budget

| 資料源 | 目前 live 規則 |
| ------ | -------------- |
| FinMind institutional flow | 只使用 all-market `TaiwanStockInstitutionalInvestorsBuySell` 查詢。當日軌道用 `date = run_date` 一次抓全市場；近期累積軌道用 `run_date - 10 calendar days` 到 `run_date` 的 date range 抓全市場，再在記憶體中取最近 5 個市場交易日排名。不得帶 `stock_id`、`data_id`、`symbol` 等逐檔參數。 |
| yfinance OHLCV | 只對 selected universe 中缺少 final `StockRawData` 的 symbol 做一次 batch download。Batch 以 `run_date - 120 days` 到 `run_date + 1 day` 取日線，再只保留 `run_date` 當日或之前的資料。已有 final raw rows 會重用，不重抓。 |

目前 live 限制：尚未做完整 live margin fetch，也尚未帶入完整 market context。為避免技術與法人資料被誤判為 stale，回填的 raw rows 只放入最小 margin `data_date`，不宣稱已取得完整融資融券內容。Market context 目前以空物件傳入，完整大盤濾網仍屬後續補強。
...
3. Daily Radar service 內部選出雙軌 universe，補齊 selected symbols 缺少的 OHLCV，執行 Stage 1/2 scoring。
...
| Request budget | FinMind 只做同日與近期區間 all-market 查詢；yfinance 只對缺資料的 selected symbols 做一次 batch download |
...
| 決策 | 預設方向 | 需確認事項 |
| ---- | -------- | ---------- |
| 掃描 universe | live default 使用雙軌 selected universe | 是否後續擴展到完整上市櫃全市場掃描 |
