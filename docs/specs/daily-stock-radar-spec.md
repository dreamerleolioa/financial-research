...
| Shared background context cache | `refresh-lending` 台灣時間 20:00 執行；`refresh-full-margin` 台灣時間 21:30 執行，等待 FinMind 21:00 後更新；兩者先重用同日 fresh cache，缺資料才呼叫 provider。`weekly_major_holders` 維持週頻 TDCC 背景排程更新，不在每日 run 內強行日更；未指定 symbols 時，其更新範圍可合併 active holdings、watchlist 與 latest Daily Radar candidates，但 lending/full margin 仍限 latest Daily Radar candidates。 |
...
Scoring 必須避免高度相關技術指標重複加分。`primary_score_inputs` 只放主要方向與可控性證據；`risk_overheat_filters` 只放過熱/高波動懲罰；`secondary_evidence` 只作輔助，不主導 primary score；`display_only` 完全不影響 score。支撐壓力 primary scoring 必須用當前 bar 之前的 20 根已完成 bar 判斷 breakdown/near-support/near-resistance；raw `technical_indicators.high_20d` / `low_20d` 可作 display 數值並包含當前 bar，但不得直接拿來判斷當前 close 是否跌破支撐。ATR 必須切成 `atr_risk` 與 `atr_state`：前者只回答支撐/停損距離是否可控，後者才處理高波動懲罰，避免同一 ATR 事實重複計票。OHLC high/low 不完整時，支撐壓力 primary signal 不計分，並在 `data_quality`/caveat 中保留原因。
