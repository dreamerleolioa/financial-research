...
MVP 掃描台股一般上市櫃股票，排除 ETF、權證、特別股與資料欄位明顯不完整標的。實際 symbol 清單可先由既有資料源或靜態清單提供，後續再做自動更新。
...
3. Daily Radar service 內部處理資料載入與必要補齊、Stage 1 prefilter、Stage 2 detailed scoring。
...
| Request budget | 記錄外部 API 呼叫量，避免全市場逐檔打昂貴端點 |
...
| 決策 | 預設方向 | 需確認事項 |
| ---- | -------- | ---------- |
| 掃描 universe | 上市櫃一般股票 | symbol 清單來源與更新頻率 |
