...
目前已落地的 live run 是自包含流程：`POST /internal/daily-radar/run` 先呼叫 multi-track universe 選股，再針對 selected symbols 刷新試驗版 Daily AVWAP snapshot，補齊缺少的 OHLCV，接著執行 Stage 1/2 rule-based scoring，最後寫入 run log 與 candidates。公開 Daily Radar 讀取端點與 response schema 不因這個後端流程改變。
...
| FinMind Phase 1 Daily AVWAP | 只對 selected universe symbols 做 single-symbol `TaiwanStockPrice` refresh；同一 `data_date` 已有 fresh snapshot 時直接重用。若 FinMind 尚未提供 requested `run_date` row，寫入 missing snapshot trace，不阻塞 Daily Radar persistence。 |
...
| Shared background context cache | Daily run 先選出 selected symbols，再針對台股 selected symbols 刷新日頻 `lending` / `full_margin` cache，最後批次讀取 `shared_background_contexts` 產生 background labels；`weekly_major_holders` 維持週頻背景排程更新，不在每日 run 內強行日更。 |
...
3. Daily Radar service 內部選出 multi-track universe，針對台股 selected symbols 刷新試驗版 Daily AVWAP snapshot 與日頻 `lending` / `full_margin` shared background context cache，批次讀取 selected symbols 的 shared background context cache，補齊 selected symbols 缺少的 OHLCV，執行 Stage 1/2 scoring。
...
Phase 2A 另有獨立 workflow `.github/workflows/daily-radar-chip-context.yml`，可呼叫 `POST /internal/daily-radar/chip-context/update`。每日正式 Daily Radar run 會在 selected universe 確定後刷新試驗版 Daily AVWAP snapshot 與 `lending` / `full_margin`，確保 candidate snapshot 盡量包含當次候選股的 AVWAP 與日頻背景脈絡；獨立 workflow 仍保留為 shared context 維護與週頻 `weekly_major_holders` 更新路徑。本機 script 只能作為除錯輔助，不是正式更新路徑。Workflow 依資料頻率拆分：日頻 `lending` / `full_margin` 僅作手動維護/補跑，手動觸發時用 `context_group` 選擇 `daily` / `weekly` / `all`；週頻 `weekly_major_holders` 在台灣時間週日 07:30 排程更新。
...
| Request budget | TWSE RWD fund reports 用 report-level 查詢建立法人 tracks；FinMind AVWAP 只對 selected symbols 做 single-symbol refresh 並重用同日 fresh snapshot；yfinance 只對缺資料的 selected symbols 做一次 batch download；market index 只抓固定 benchmark；Daily Radar run 只對 selected symbols 刷新日頻 lending/full margin；weekly major holders 由週頻背景 endpoint 更新 cache |
| Stale guard | 若核心資料日期過舊，run status 應標示 `stale_data` |
| Background context failure | Daily Radar run 內的 AVWAP 或日頻背景刷新失敗時以 log 與 missing/stale cache trace 降級，不阻塞 candidate persistence；獨立 chip-context workflow 對非 completed 業務狀態 fail job 以利監控 |
...
| Background context tests | migration/model、repository upsert/read、internal updater endpoint、workflow request budget、daily run 先刷新日頻 selected-symbol context 後讀 cache，且 background labels 不改 ranking |
