...
- **Partial provider TTL**：provider 即使回傳非空資料，少於 MA60 所需的 60 根可用交易 bar 時仍必須標記 `provider_coverage_insufficient`，並使用 5 分鐘失敗短 TTL；不得因至少一根 bar 就套用 6 小時成功 TTL。
...
- **OHLC 與 full-exit 路徑邊界**：逐序列完成 event-date 裁切後，需要 OHLC 的指標與 compact evidence 必須再取 Close／High／Low 的共同交易日，禁止按索引拼接獨立 `dropna()` 後的序列。Full-exit 只有日期而無成交時間時，結案日收盤不得進入 lifecycle 已持有路徑；實際出場價只使用 ledger event fact。
