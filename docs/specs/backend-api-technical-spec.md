...
- 23:30 TWT：`POST /internal/daily-radar/refresh-market-context`，以 yfinance batch download 取得指數，失敗、空資料或 stale 時再使用同 provider 的 `Ticker.history` bounded fallback，並保存 `provider_trace`。只有 record date 正確、fresh 且 regime 可判定的 market context 才把 step 標成 `completed`；missing/stale/指標不足必須標 `failed` 並阻擋 scoring。若同一 run 已有通過驗證的 context，暫時性重抓失敗沿用既有資料且不得用缺漏 payload 覆寫。
