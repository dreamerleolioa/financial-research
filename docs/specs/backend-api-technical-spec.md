...
`run-scoring` 不得打 FinMind、yfinance、TWSE 或 market index provider；缺少 prepared universe、prepared market context、final raw rows，或任一 required refresh step 不是 `completed` 時回 `409`，由 workflow/monitor 顯示資料準備缺口。Required refresh steps 為 `refresh-lending`、`refresh-full-margin`、`refresh-ohlcv`、`refresh-market-context`。`refresh-avwap` 是 optional evidence step：失敗或缺漏不得阻塞 `run-scoring`，但 candidate detail 必須保留 `input_snapshot.phase1_avwap_context.freshness`、`missing_reason` 與 data-quality caveat。
...
- 一般分析校準的 active cohort 固定為目前 `strategy_version` + `confidence_config_version`；資料庫唯一鍵包含 `analysis_type / market / symbol / record_date / strategy_version / confidence_config_version`，日內重跑不得因 input hash 改變而增加獨立樣本。升級 migration 會以 exclusive table lock 阻止 writer 競態，lock 等待上限固定為 10 秒，逾時必須 fail closed 並由 operator 排除阻塞 transaction 後重試；同一 identity 先一次性固定具有最多 `validated` outcomes 的 canonical sample，再以 evaluated outcome 數與最早 ID 決定 tie-break。只保留 canonical sample 原生 outcomes，絕不把其他 input hash 的 outcome 改掛過來；缺少的窗口由後續 due validation 重算。此 canonicalization 刻意不可 downgrade，部署前必須備份、停止所有舊版 backend 與 calibration workflows，並設定一次性 `CALIBRATION_MIGRATION_BACKUP_CONFIRMED=2c3d4e5f6a7b`，否則 upgrade fail closed。
...
- `coverage` 或 `replay_coverage`：distinct samples、整體與逐月 replay coverage、排除原因及 threshold 結果。一般分析另輸出 `baseline_replay_complete`；Daily Radar 的 `replay_workload` 輸出 candidate／row／config／ablation 數量、estimated scoring calls、bootstrap row-iterations、各上限及 exceeded limits。
