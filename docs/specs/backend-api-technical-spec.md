...
- `POST /internal/fundamentals/backfill`：對 request `symbols` 或 managed universe 做 FinMind 歷史 bootstrap；managed universe 合併 active holdings、watchlist、最新 prepared universe，以及最近一次 `refresh-ai-evidence=completed` 日期全部 final、支援的 `.TW` / `.TWO` AI raw-pool symbols，避免尚在 materialize 的日期被誤當完整 pool。第一頁以批次查詢排除 EPS 與股利 lane 都已完整的 symbols，再把剩餘 immutable symbol snapshot、`raw_pool_date`、server-owned `next_after_symbol` 與狀態保存到 `fundamental_backfill_jobs`，response 回傳 `job_id`；後續頁與跨 workflow 續跑必須同時帶回 `job_id`、`next_after_symbol`，API 以 row lock 驗證 cursor 等於 job 記錄值，並只在整頁成功後原子前移，不得重建 live managed universe；completed job 是終止狀態，不得重新呼叫上游。只有 `after_symbol` 卻沒有 `job_id` 時回 `422 fundamental_backfill_job_id_required`，找不到 job 時回 `404 fundamental_backfill_job_not_found`，completed job 或 cursor 不一致時回 `409`；managed scope 明確指定的 `raw_pool_date` 也必須已有 `refresh-ai-evidence=completed`，否則回 `409 fundamental_backfill_raw_pool_not_completed`，完成日期沒有 final 支援台股資料時回 `409 fundamental_backfill_raw_pool_not_found`。每次最多 10 檔，只有快取仍缺足夠 EPS 歷史或完整年度股利的 lane 才呼叫對應 FinMind dataset，已覆蓋 lane 不重複消耗 quota；任一 lane 失敗時 `status=partial` 且不前移 cursor，下一次以同一 job 從原 cursor 重試。GitHub workflow 必須先擷取並輸出 job/cursor 再處理 partial failure；每次最多六批，未完成時以 `BACKFILL_NEXT_AFTER_SYMBOL` 與 `BACKFILL_JOB_ID` 回報，下一次須同時填入 `backfill_after_symbol` 與 `backfill_job_id`，不得重新由第一頁開始。
