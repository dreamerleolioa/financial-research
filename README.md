...
Daily Radar 另有 GitHub Actions workflow，可手動執行或於台灣市場交易日收盤後排程執行。正式 workflow 會分段 POST 到 `${ZEABUR_BACKEND_URL}/internal/daily-radar/*`，用 `DAILY_RADAR_INTERNAL_TOKEN` 做內部 API 驗證：台灣時間 18:00 `prepare-universe` 保存 capped 250 selected symbols；19:00 `refresh-avwap`；20:00 `refresh-lending`；21:30 `refresh-full-margin`；22:30 `refresh-ohlcv`；23:30 `refresh-market-context`；隔日 00:30 `run-scoring` 只讀 DB cache/snapshot 並持久化 Daily Radar candidates。`run-scoring` 會檢查 prepared run 的每個 refresh step 都是 `completed`；任一步缺失或 failed 時回 `409`，避免用不完整資料發佈雷達。
