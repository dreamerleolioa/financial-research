...
- `/portfolio`：持股、加碼、結案、事件 ledger、進場脈絡、lifecycle plan、single trade review 與 group-level lifecycle review。結案回顧採 closed-only、以前一 completed bar、source fingerprint、版本唯讀保護與短 transaction 並行鎖為契約；review provider 補行情具 timeout／容量／TTL／coverage 邊界，外部 I/O 不持有 DB lock 且不寫回正式 `StockRawData`，事後補填或進場後已修改的 plan 也不參與歷史違規或決策品質評分。
