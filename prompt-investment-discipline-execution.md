...
本 phase 目標：
建立 deterministic forward validation workflow，衡量 Daily Radar candidates 在 5/10/20 個 trading days 後的 forward behavior。這是 rule-quality / calibration 診斷，不是績效宣傳，不改 live scoring。

正式資料來源要求：
正式 validation 必須由雲端 backend 使用 production DB / production market data 產生。本地 CLI 只作 fixture、local development、debug、regression test，不是正式月報來源。
...
2. 新增 cloud internal API，例如：
   - POST /internal/daily-radar/forward-validation/run
3. 支援 fixture/local mode 與 cloud production mode。若資料缺漏，必須回報 skip reasons。
4. API 支援 `mode: due`，由 `as_of_date` 盤點哪些歷史 candidates 已滿 5/10/20 trading-day windows。
5. validation result 寫入必須 idempotent，例如用 candidate/window/validation_version 或等價 key 防止重複寫入。
6. 支援 windows：5、10、20 trading days。
7. 至少計算：
   - forward_return_pct
...
9. 輸出 stable JSON，sorted keys，無 wall-clock timestamp。
10. Report shape 至少包含：
   - metadata
   - sample_summary
   - bucket_outcomes
   - rule_outcomes
   ...
   - skip_reasons
   - version_manifest
11. 新增文件說明此 report 是 rule-quality / calibration 診斷，不是績效宣傳。
...
- API tests：due-window selection、idempotent writes、production-mode skip reasons。
...
- fixture report 摘要。
- db mode 是否可用與限制。
- cloud internal API 觸發方式與 response 摘要。
- 跑了哪些測試/QA。
...
本 phase 目標：
建立 rule registry、rule tier、validation status 與 ablation workflow，讓 scoring signals 可以被升級、降級、移到 context only 或 deprecated。另建立 monthly cloud rule-review report 交付流程，讓使用者可以下載報告並交給後續 agent 產出 versioned strategy update plan。

正式報告來源要求：
正式 monthly rule-review report 必須由雲端 backend 使用 production validation results 產出。本地 CLI 只作開發、fixture replay、debug、回歸測試用途。
...
8. 新增 monthly rule-review internal API，例如：
   - POST /internal/daily-radar/rule-review/monthly
9. Monthly API 從 production validation results 彙總，不直接依賴本地 fixture。
10. Monthly report 至少輸出 JSON 與 Markdown，供使用者交給 agent review。
11. 設計 GitHub Actions artifact 交付方式：workflow 呼叫雲端 internal API，保存 JSON/Markdown artifacts。
12. Monthly report 必須只產生 recommendation，不自動修改 live scoring。
...
7. Monthly rule-review API 能從 production validation results 產生 JSON/Markdown report。
8. GitHub Actions 或等價 cloud workflow 能保存 downloadable artifacts。
9. Report 明確區分 automated recommendation 和 human-approved versioned strategy update。
...
- Artifact-generation workflow docs/checks；若 workflow 實作，需有 workflow syntax/basic smoke validation。
...
- ablation 摘要。
- monthly report API 與 artifact 交付方式。
- 跑了哪些測試/QA。
- 下一個建議 phase。
```
