...
def test_event_cost_inputs_reject_more_than_two_decimal_places(model, payload, field):
    with pytest.raises(ValidationError):
        model(**payload, **{field: 1.001})
...
def test_lifecycle_plan_inputs_match_postgresql_numeric_upper_bounds(field, maximum):
    assert getattr(BackfillLifecyclePlanRequest(**{field: float(maximum)}), field) == maximum
...
@pytest.mark.parametrize(("field", "value"), [
    ("planned_stop_price", 95.00001),
    ("planned_risk_amount", 1.001),
    ("planned_risk_pct", 1.00001),
])
def test_lifecycle_plan_inputs_reject_values_beyond_postgresql_scale(field, value):
    with pytest.raises(ValidationError):
        BackfillLifecyclePlanRequest(**{field: value})
