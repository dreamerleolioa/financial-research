from __future__ import annotations

import json
import logging
import time
from datetime import date
from types import SimpleNamespace
from typing import Any

from ai_stock_sentinel.data_sources import official_http
from ai_stock_sentinel.daily_radar import institutional_evidence
from ai_stock_sentinel.daily_radar.institutional_evidence import (
    OfficialInstitutionalEvidenceProvider,
    TPEX_3I_URL,
    TWSE_T86_FALLBACK_URL,
    TWSE_T86_URL,
    _build_payload,
    cached_daily_rows_from_raw_rows,
)


class _Response:
    def __init__(
        self,
        payload: dict[str, Any],
        *,
        content: bytes = b"{}",
        content_type: str = "application/json",
    ) -> None:
        self._payload = payload
        self.content = content
        self.headers = {"content-type": content_type}
        self.status_code = 200

    def raise_for_status(self) -> None:
        return None

    def json(self) -> dict[str, Any]:
        return self._payload


def test_twse_institutional_evidence_uses_official_report_route() -> None:
    assert TWSE_T86_URL == "https://www.twse.com.tw/fund/T86"
    assert TWSE_T86_FALLBACK_URL == "https://www.twse.com.tw/rwd/zh/fund/T86"


def test_twse_institutional_evidence_retries_transient_timeout(
    caplog,
) -> None:
    calls: list[str] = []
    timeouts: list[float] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append(url)
        timeouts.append(timeout)
        if len(calls) == 1:
            raise TimeoutError("temporary TWSE timeout")
        row: list[Any] = [""] * 19
        row[0] = "2330"
        return _Response({"stat": "OK", "data": [row]})

    with caplog.at_level(logging.WARNING):
        result = OfficialInstitutionalEvidenceProvider(
            request_get=request_get,
            recent_market_days=1,
            calendar_window_days=0,
        ).fetch(["2330.TW"], run_date=date(2026, 8, 13))

    assert calls == [TWSE_T86_URL, TWSE_T86_FALLBACK_URL]
    assert timeouts == [5.0, 5.0]
    assert result.errors == []
    assert set(result.payloads_by_symbol) == {"2330.TW"}
    events = [json.loads(record.message) for record in caplog.records]
    assert [event["event"] for event in events] == [
        "institutional_evidence_request_retry",
        "institutional_evidence_request_recovered",
    ]
    shared_fields = {
        "provider": "official_twse_tpex",
        "market": "TWSE",
        "dataset": "institutional_flow",
        "query_date": "2026-08-13",
        "max_attempts": 2,
    }
    assert all(shared_fields.items() <= event.items() for event in events)
    assert events[0]["endpoint_url"] == TWSE_T86_URL
    assert events[0]["error_type"] == "TimeoutError"
    assert events[1]["endpoint_url"] == TWSE_T86_FALLBACK_URL


def test_twse_institutional_evidence_retries_invalid_json(caplog) -> None:
    calls: list[str] = []

    class InvalidJsonResponse(_Response):
        def json(self) -> dict[str, Any]:
            raise json.JSONDecodeError("temporary non-JSON body", "<html>", 0)

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append(url)
        if len(calls) == 1:
            return InvalidJsonResponse(
                {},
                content=b"<html>upstream failure</html>",
                content_type="text/html",
            )
        row: list[Any] = [""] * 19
        row[0] = "2330"
        return _Response({"stat": "OK", "data": [row]})

    with caplog.at_level(logging.WARNING):
        result = OfficialInstitutionalEvidenceProvider(
            request_get=request_get,
            recent_market_days=1,
            calendar_window_days=0,
        ).fetch(["2330.TW"], run_date=date(2026, 8, 13))

    assert calls == [TWSE_T86_URL, TWSE_T86_FALLBACK_URL]
    assert result.errors == []
    assert set(result.payloads_by_symbol) == {"2330.TW"}
    retry_event = json.loads(caplog.records[0].message)
    assert retry_event["content_type"] == "text/html"
    assert retry_event["response_bytes"] == 29
    assert "upstream failure" not in caplog.text
    assert "response_body" not in caplog.text


def test_twse_institutional_evidence_retries_transient_http_status(caplog) -> None:
    calls: list[str] = []

    class ServiceUnavailableResponse(_Response):
        def __init__(self) -> None:
            super().__init__(
                {},
                content=b"service unavailable",
                content_type="text/plain",
            )
            self.status_code = 503

        def raise_for_status(self) -> None:
            raise RuntimeError("HTTP 503")

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append(url)
        if len(calls) == 1:
            return ServiceUnavailableResponse()
        row: list[Any] = [""] * 19
        row[0] = "2330"
        return _Response({"stat": "OK", "data": [row]})

    with caplog.at_level(logging.WARNING):
        result = OfficialInstitutionalEvidenceProvider(
            request_get=request_get,
            recent_market_days=1,
            calendar_window_days=0,
        ).fetch(["2330.TW"], run_date=date(2026, 8, 13))

    assert result.errors == []
    assert calls == [TWSE_T86_URL, TWSE_T86_FALLBACK_URL]
    retry_event = json.loads(caplog.records[0].message)
    assert retry_event["http_status"] == 503
    assert retry_event["content_type"] == "text/plain"
    assert retry_event["response_bytes"] == 19


def test_official_institutional_evidence_projects_twse_and_tpex_rows() -> None:
    calls: list[tuple[str, dict[str, str]]] = []
    twse_row: list[Any] = [""] * 19
    twse_row[0] = "2330"
    twse_row[4] = "1,000"
    twse_row[10] = "200"
    twse_row[11] = "100"
    twse_row[18] = "1,300"
    tpex_row: list[Any] = [""] * 24
    tpex_row[0] = "2454"
    tpex_row[4] = "2,000"
    tpex_row[13] = "300"
    tpex_row[22] = "-100"
    tpex_row[23] = "2,200"

    def request_get(url: str, *, params: dict[str, str], timeout: int) -> _Response:
        assert timeout == 5
        calls.append((url, params))
        if url == TWSE_T86_URL:
            return _Response({"stat": "OK", "data": [twse_row]})
        assert url == TPEX_3I_URL
        return _Response({"stat": "ok", "tables": [{"data": [tpex_row]}]})

    provider = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        calendar_window_days=0,
    )
    result = provider.fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert result.errors == []
    assert set(result.payloads_by_symbol) == {"2330.TW", "2454.TWO"}
    assert result.payloads_by_symbol["2330.TW"]["three_party_net_shares"] == 1300.0
    assert result.payloads_by_symbol["2454.TWO"]["foreign_net_shares"] == 2000.0
    assert result.payloads_by_symbol["2454.TWO"]["flow_state"] == "same_day_net_buy"
    params_by_url = {url: params for url, params in calls}
    assert params_by_url[TWSE_T86_URL]["date"] == "20260813"
    assert params_by_url[TPEX_3I_URL]["d"] == "115/08/13"


def test_default_official_client_uses_curl_compatible_transport(monkeypatch) -> None:
    tpex_row: list[Any] = [""] * 24
    tpex_row[0] = "2454"
    tpex_row[4] = "2,000"
    tpex_row[13] = "300"
    tpex_row[22] = "-100"
    tpex_row[23] = "2,200"
    calls: list[str] = []

    sessions: list[Any] = []

    class FakeSession:
        def __init__(self) -> None:
            self.closed = False
            sessions.append(self)

        def get(self, url: str, **_kwargs: Any) -> _Response:
            calls.append(url)
            if url == TWSE_T86_URL:
                return _Response({"stat": "OK", "data": []})
            assert url == TPEX_3I_URL
            return _Response({"stat": "ok", "tables": [{"data": [tpex_row]}]})

        def close(self) -> None:
            self.closed = True

    monkeypatch.setattr(official_http.curl_requests, "Session", FakeSession)
    assert institutional_evidence._import_requests_get() is official_http.official_request_get

    provider = OfficialInstitutionalEvidenceProvider(calendar_window_days=0)
    result = provider.fetch(["2454.TWO"], run_date=date(2026, 8, 18))

    assert result.errors == []
    assert calls == [TPEX_3I_URL]
    assert len(sessions) == 1
    assert sessions[0].closed is True
    assert result.payloads_by_symbol["2454.TWO"]["data_dates"] == {
        "institutional_flow": "2026-08-18"
    }


def test_institutional_evidence_stops_after_each_market_has_recent_days() -> None:
    calls: list[tuple[str, str]] = []

    def request_get(url: str, *, params: dict[str, str], timeout: int) -> _Response:
        assert timeout == 5
        query_date = params.get("date") or params["d"]
        calls.append((url, query_date))
        if url == TWSE_T86_URL:
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})
        row = [""] * 24
        row[0] = "2454"
        return _Response({"stat": "ok", "tables": [{"data": [row]}]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=2,
        calendar_window_days=10,
    ).fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert result.errors == []
    assert set(calls[:2]) == {
        (TWSE_T86_URL, "20260813"),
        (TPEX_3I_URL, "115/08/13"),
    }
    assert set(calls[2:]) == {
        (TWSE_T86_URL, "20260812"),
        (TPEX_3I_URL, "115/08/12"),
    }


def test_institutional_evidence_reuses_cached_history_without_request() -> None:
    def request_get(*_args: Any, **_kwargs: Any) -> _Response:
        raise AssertionError("complete cached history must not call the provider")

    source_dates = [
        date(2026, 8, 19),
        date(2026, 8, 18),
        date(2026, 8, 17),
        date(2026, 8, 14),
        date(2026, 8, 13),
    ]
    cached = {
        "2330.TW": {
            source_date: {"foreign": 10, "trust": 20, "dealer": 30, "total": 60}
            for source_date in source_dates
        }
    }
    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=5,
        calendar_window_days=10,
    ).fetch(
        ["2330.TW"],
        run_date=date(2026, 8, 19),
        cached_daily_rows=cached,
    )

    assert result.errors == []
    assert result.payloads_by_symbol["2330.TW"]["recent_source_dates"] == [
        item.isoformat() for item in reversed(source_dates)
    ]


def test_institutional_evidence_refetches_partially_cached_market_date() -> None:
    calls: list[str] = []
    cached = {
        "2330.TW": {
            date(2026, 8, 19): {
                "foreign": 10,
                "trust": 20,
                "dealer": 30,
                "total": 60,
            }
        }
    }

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append(params["date"])
        first_row: list[Any] = [""] * 19
        first_row[0] = "2330"
        second_row: list[Any] = [""] * 19
        second_row[0] = "2454"
        return _Response({"stat": "OK", "data": [first_row, second_row]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=1,
        calendar_window_days=0,
    ).fetch(
        ["2330.TW", "2454.TW"],
        run_date=date(2026, 8, 19),
        cached_daily_rows=cached,
    )

    assert calls == ["20260819"]
    assert result.errors == []
    assert set(result.payloads_by_symbol) == {"2330.TW", "2454.TW"}


def test_cached_daily_rows_rejects_future_observations() -> None:
    raw_row = SimpleNamespace(
        symbol="2330.TW",
        record_date=date(2026, 8, 19),
        institutional={
            "official_daily_flow": {
                "source_provider": "official_twse_tpex",
                "rows": [
                    {
                        "source_date": "2026-08-20",
                        "foreign_net_shares": 10.0,
                        "investment_trust_net_shares": 20.0,
                        "dealer_net_shares": 30.0,
                        "three_party_net_shares": 60.0,
                    }
                ],
            }
        },
    )

    assert cached_daily_rows_from_raw_rows([raw_row]) == {}


def test_institutional_evidence_skips_weekends() -> None:
    called_dates: list[str] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        assert url == TWSE_T86_URL
        called_dates.append(params["date"])
        row: list[Any] = [""] * 19
        row[0] = "2330"
        return _Response({"stat": "OK", "data": [row]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=2,
        calendar_window_days=4,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 17))

    assert result.errors == []
    assert called_dates == ["20260817", "20260814"]


def test_institutional_evidence_opens_circuit_after_same_host_timeout() -> None:
    calls: list[tuple[str, str]] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append((url, params["date"]))
        raise TimeoutError("TWSE silent timeout")

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=5,
        calendar_window_days=10,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 19))

    assert calls == [
        (TWSE_T86_URL, "20260819"),
        (TWSE_T86_FALLBACK_URL, "20260819"),
    ]
    assert result.errors == [
        {
            "market": "TWSE",
            "query_date": "2026-08-19",
            "error_type": "TimeoutError",
        },
        {
            "market": "TWSE",
            "error_type": "institutional_evidence_lookback_incomplete",
            "market_day_count": 0,
            "required_market_day_count": 5,
        },
    ]


def test_institutional_evidence_opens_circuit_after_retryable_http_failure() -> None:
    calls: list[tuple[str, str]] = []

    class ServiceUnavailableResponse(_Response):
        def __init__(self) -> None:
            super().__init__({}, content=b"unavailable", content_type="text/plain")
            self.status_code = 503

        def raise_for_status(self) -> None:
            raise RuntimeError("HTTP 503")

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append((url, params["date"]))
        return ServiceUnavailableResponse()

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=5,
        calendar_window_days=10,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 19))

    assert calls == [
        (TWSE_T86_URL, "20260819"),
        (TWSE_T86_FALLBACK_URL, "20260819"),
    ]
    assert [error["error_type"] for error in result.errors] == [
        "RuntimeError",
        "institutional_evidence_lookback_incomplete",
    ]


def test_institutional_evidence_fails_closed_when_current_date_has_no_rows() -> None:
    calls: list[str] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        calls.append(url)
        return _Response({"stat": "OK", "data": []})

    cached_dates = [
        date(2026, 8, 18),
        date(2026, 8, 17),
        date(2026, 8, 14),
        date(2026, 8, 13),
        date(2026, 8, 12),
    ]
    cached = {
        "2330.TW": {
            source_date: {"foreign": 10, "trust": 20, "dealer": 30, "total": 60}
            for source_date in cached_dates
        }
    }
    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=5,
        calendar_window_days=10,
    ).fetch(
        ["2330.TW"],
        run_date=date(2026, 8, 19),
        cached_daily_rows=cached,
    )

    assert calls == [TWSE_T86_URL, TWSE_T86_FALLBACK_URL]
    assert result.errors == [
        {
            "market": "TWSE",
            "query_date": "2026-08-19",
            "error_type": "InstitutionalEvidenceEmptyResponse",
        }
    ]
    assert result.payloads_by_symbol["2330.TW"]["data_dates"] == {
        "institutional_flow": "2026-08-18"
    }


def test_institutional_evidence_keeps_non_transport_historical_error() -> None:
    def request_get(url: str, *, params: dict[str, str], timeout: int) -> _Response:
        assert timeout == 5
        if url == TWSE_T86_URL and params["date"] == "20260812":
            raise ValueError("invalid historical payload")
        if url == TWSE_T86_URL:
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})
        row = [""] * 24
        row[0] = "2454"
        return _Response({"stat": "ok", "tables": [{"data": [row]}]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=2,
        calendar_window_days=3,
    ).fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert result.errors == [
        {
            "market": "TWSE",
            "query_date": "2026-08-12",
            "error_type": "ValueError",
        }
    ]


def test_institutional_evidence_caps_request_timeout_to_remaining_deadline(
    monkeypatch,
) -> None:
    clock = iter([100.0, 102.0])
    monkeypatch.setattr(institutional_evidence, "monotonic", lambda: next(clock))
    timeouts: list[float] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        timeouts.append(timeout)
        if url == TWSE_T86_URL:
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})
        row = [""] * 24
        row[0] = "2454"
        return _Response({"stat": "ok", "tables": [{"data": [row]}]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=1,
        calendar_window_days=0,
        total_timeout=3,
    ).fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert result.errors == []
    assert timeouts == [0.5, 0.5]


def test_institutional_evidence_recomputes_deadline_for_queued_markets() -> None:
    timeouts: list[float] = []

    def request_get(url: str, *, params: dict[str, str], timeout: float) -> _Response:
        timeouts.append(timeout)
        time.sleep(min(0.7, timeout))
        if url == TWSE_T86_URL:
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})
        row = [""] * 24
        row[0] = "2454"
        return _Response({"stat": "ok", "tables": [{"data": [row]}]})

    OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=1,
        calendar_window_days=0,
        max_workers=1,
        total_timeout=1,
    ).fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert len(timeouts) == 2
    assert timeouts[1] < 0.5


def test_institutional_evidence_keeps_current_date_timeout_as_error() -> None:
    def request_get(url: str, *, params: dict[str, str], timeout: int) -> _Response:
        assert timeout == 5
        if (
            url in {TWSE_T86_URL, TWSE_T86_FALLBACK_URL}
            and params["date"] == "20260813"
        ):
            raise TimeoutError("current TWSE timeout")
        if url == TWSE_T86_URL:
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})
        row = [""] * 24
        row[0] = "2454"
        return _Response({"stat": "ok", "tables": [{"data": [row]}]})

    result = OfficialInstitutionalEvidenceProvider(
        request_get=request_get,
        recent_market_days=2,
        calendar_window_days=3,
    ).fetch(["2330.TW", "2454.TWO"], run_date=date(2026, 8, 13))

    assert result.errors == [
        {
            "market": "TWSE",
            "query_date": "2026-08-13",
            "error_type": "TimeoutError",
        },
        {
            "market": "TWSE",
            "error_type": "institutional_evidence_lookback_incomplete",
            "market_day_count": 0,
            "required_market_day_count": 2,
        },
    ]


def test_institutional_evidence_default_client_uses_two_bounded_attempts_per_date(
    monkeypatch,
) -> None:
    calls = 0

    class FakeSession:
        def get(self, *_args: Any, **_kwargs: Any) -> _Response:
            nonlocal calls
            calls += 1
            raise official_http.curl_requests.exceptions.Timeout("official timeout")

        def close(self) -> None:
            return None

    monkeypatch.setattr(official_http.curl_requests, "Session", FakeSession)
    monkeypatch.setattr(official_http.time, "sleep", lambda _seconds: None)
    result = OfficialInstitutionalEvidenceProvider(
        calendar_window_days=0,
        total_timeout=5,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 13))

    assert calls == 2
    assert result.errors == [
        {
            "market": "TWSE",
            "query_date": "2026-08-13",
            "error_type": "Timeout",
        },
        {
            "market": "TWSE",
            "error_type": "institutional_evidence_lookback_incomplete",
            "market_day_count": 0,
            "required_market_day_count": 1,
        },
    ]


def test_institutional_evidence_reuses_session_and_paces_market_dates(
    monkeypatch,
) -> None:
    sessions: list[Any] = []
    sleeps: list[float] = []

    class FakeSession:
        def __init__(self) -> None:
            self.calls: list[str] = []
            self.closed = False
            sessions.append(self)

        def get(self, url: str, *, params: dict[str, str], timeout: float) -> _Response:
            assert url == TWSE_T86_URL
            self.calls.append(params["date"])
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})

        def close(self) -> None:
            self.closed = True

    monkeypatch.setattr(official_http.curl_requests, "Session", FakeSession)
    monkeypatch.setattr(institutional_evidence.time, "sleep", sleeps.append)

    result = OfficialInstitutionalEvidenceProvider(
        recent_market_days=2,
        calendar_window_days=2,
        min_request_interval_seconds=0.75,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 18))

    assert result.errors == []
    assert len(sessions) == 1
    assert sessions[0].calls == ["20260818", "20260817"]
    assert sessions[0].closed is True
    assert len(sleeps) == 1
    assert 0 < sleeps[0] <= 0.75


def test_institutional_evidence_pacing_respects_global_deadline(monkeypatch) -> None:
    calls = 0

    class FakeSession:
        def get(self, url: str, *, params: dict[str, str], timeout: float) -> _Response:
            nonlocal calls
            assert url == TWSE_T86_URL
            calls += 1
            row: list[Any] = [""] * 19
            row[0] = "2330"
            return _Response({"stat": "OK", "data": [row]})

        def close(self) -> None:
            return None

    monkeypatch.setattr(official_http.curl_requests, "Session", FakeSession)
    started_at = time.monotonic()
    result = OfficialInstitutionalEvidenceProvider(
        recent_market_days=2,
        calendar_window_days=2,
        total_timeout=1,
        min_request_interval_seconds=2,
    ).fetch(["2330.TW"], run_date=date(2026, 8, 18))
    elapsed = time.monotonic() - started_at

    assert calls == 1
    assert elapsed < 1.2
    assert any(
        error["error_type"] == "institutional_evidence_total_timeout"
        for error in result.errors
    )


def test_institutional_evidence_does_not_label_prior_date_as_same_day() -> None:
    payload = _build_payload(
        "2330.TW",
        {
            date(2026, 8, 12): {
                "foreign": 1000.0,
                "trust": 200.0,
                "dealer": 100.0,
                "total": 1300.0,
            }
        },
        run_date=date(2026, 8, 13),
        active_dates=[date(2026, 8, 13), date(2026, 8, 12)],
    )

    assert "same_day_actor" not in payload
    assert "same_day_net_buy" not in payload
    assert payload["data_dates"]["institutional_flow"] == "2026-08-12"
