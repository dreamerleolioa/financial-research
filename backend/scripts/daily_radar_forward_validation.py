...
def main() -> None:
    ...
    if args.source == "fixture":
        ...
        with _get_session_local()() as session:
            ...
            price_series = load_price_series_from_raw_data(
                session,
                symbols=sorted(symbols | {benchmark_symbol}),
                start_date=start_date,
    ...
    if args.output is not None:
        write_report(evaluation.report, args.output)
    else:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a deterministic Daily Radar forward validation report.",
    )
    ...
    parser.add_argument(
        "--write-results",
        action="store_true",
        help="Persist db-mode validation rows. Official production writes should normally use the internal API.",
    )
